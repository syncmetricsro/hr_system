...
if not django_apps.is_installed("features.messaging"):
    pytest.skip(
        "features.messaging is not installed for this client", allow_module_level=True
    )
...
def test_send_sms_records_sent_when_provider_ok(monkeypatch, make_user):
    ...
    msg = services.send_sms(
        person.phone, "Hello", actor=make_user("manager"), person=person
    )
...
def test_send_view_denied_for_observer(client, make_user):
    ...
    assert (
        client.post(reverse("send_sms", args=[person.pk]), {"body": "hi"}).status_code
        == 403
    )


def test_coordinator_cannot_message_outside_their_projects(
    client, make_user, monkeypatch
):
    monkeypatch.setattr(services, "_twilio_send", lambda to, body: "SM1")
    coord = make_user("coordinator")
    person = Person.objects.create(
        first_name="A", last_name="B", phone="+421900000000"
    )  # no project link
...
def test_coordinator_can_message_own_project_person(client, make_user, monkeypatch):
    ...
    assert OutboundMessage.objects.filter(
        person=person, status=OutboundMessage.Status.SENT
    ).exists()
