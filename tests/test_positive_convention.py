...
def month(django_user_model):
    ...
    return (
        actor,
        FinancialMonth.objects.create(project=project, year=2026, month=5),
        project,
    )
...
def test_positive_net_is_revenue_minus_cost(month):
    ...
    wage = FinanceCategory.objects.create(
        label="Gross wage", kind=FinanceCategoryKind.COST
    )
    invoice = FinanceCategory.objects.create(
        label="Client invoices", kind=FinanceCategoryKind.REVENUE
    )
...
def test_room_rate_rejects_negative(django_user_model):
    actor = django_user_model.objects.create_user(
        email="r@demo.jober.test", password="x", role="manager"
    )
    room = Room.objects.create(
        accommodation=Accommodation.objects.create(name="U"), label="1"
    )
...
def test_assignment_rate_rejects_negative(django_user_model):
    ...
    room = Room.objects.create(
        accommodation=Accommodation.objects.create(name="U"), label="1", capacity=1
    )
