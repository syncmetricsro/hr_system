...
def test_manager_cannot_view_another_offices_month_detail(client, two_offices):
    ...
    client.force_login(two_offices["manager"])
    other_month = FinancialMonth.objects.get(
        project=two_offices["p_gyr"], year=2026, month=5
    )
...
def test_manager_can_view_their_own_offices_month_detail(client, two_offices):
    ...
    client.force_login(two_offices["manager"])
    own_month = FinancialMonth.objects.get(
        project=two_offices["p_vm"], year=2026, month=5
    )
...
def test_office_totals_offices_none_returns_unfiltered_including_no_office_project(
    two_offices,
):
    ...
    record_financial_month(
        unassigned_project, 2026, 5, "500", "100", actor=two_offices["manager"]
    )
