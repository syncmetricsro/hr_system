...
def age_warning(birth_date: date | None, *, on_date: date | None = None) -> dict | None:
    ...
    if days_until > 30:
        return {
            ...
            "title": _("Under 18"),
            "message": _(
                "This person is under 18. Review the applicable employment requirements."
            ),
        }
    if 0 < days_until <= 30:
        return {
...
def inactive_by_reason(*, include_archived: bool = False, offices=None) -> list[dict]:
    ...
    ``offices=None`` means unfiltered (ADR 0026 Phase B convention, matching
...
def recycle_to_available(person, *, actor=None, reason: str = ""):
    ...
    person.set_status(
        LifecycleStatus.AVAILABLE, actor=actor, reason=reason or "recycled"
    )
...
def person_history(person) -> list[dict]:
        ...
        events.append(
            {
                ...
                "label": _("Trial scheduled"),
                "detail": trial.project.name,
            }
        )
    ...
    for room in person.room_assignments.select_related("room__accommodation"):
        events.append(
            {
                "when": room.created_at,
                "label": _("Room assigned"),
                "detail": str(room.room),
            }
        )
    ...
    for readiness in person.readiness_records.select_related("project"):
        if readiness.submitted_at:
            events.append(
                {
                    "when": readiness.submitted_at,
                    "label": _("Readiness submitted"),
                    "detail": readiness.project.name,
                }
            )
    ...
    for intake in person.intakes.all():
        if intake.completed_at:
            events.append(
                {
                    "when": intake.completed_at,
                    "label": _("Intake completed"),
                    "detail": "",
                }
            )
    ...
    lifecycle = AuditEvent.objects.filter(
        ...
        action="person.lifecycle_changed",
    )
        ...
        events.append(
            {
                ...
                % {
                    "from_status": _lifecycle_status_label(
                        event.metadata.get("from_status")
                    ),
                    "to_status": _lifecycle_status_label(
                        event.metadata.get("to_status")
                    ),
                },
            }
        )
