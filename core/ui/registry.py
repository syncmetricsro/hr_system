...
# Each: {"provider": fn(request, person) -> {"label", "periods"}|None,
#        "order": int}. Core merges the feature-owned series by period.
...
def person_finance_series(request, person) -> list[dict]:
    rendered = []
    for entry in sorted(_person_finance_series, key=lambda item: item["order"]):
        series = entry["provider"](request, person)
        if series is not None:
            rendered.append(series)
    return rendered
