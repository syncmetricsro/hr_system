...
11. **High — no in-app user or credential management at all.** Confirmed
    ...
    accounts throughout. **Corvinum authority was decided on 2026-08-07:**
    Observer and Manager may invite Recruiter, Coordinator, or Manager;
    nobody may invite Observer. This must be a narrow invitation capability,
    not an Observer grant of broad `user.manage`. Secure acceptance,
    credential lifecycle, audit, Manager TOTP enrolment, and the deployment-
    bootstrapped first Observer remain a Corvinum production blocker. Canonical
    decision: `docs/product/corvinum-account-onboarding.md`.
...
| Corvinum account onboarding | ⚠️ Open — blocks production users (2026-08-07) | Role matrix approved: Observer and Manager may invite Recruiter, Coordinator, or Manager; nobody may invite Observer. Implement a dedicated, audited, expiring single-use invitation flow plus password recovery, deactivation, lost-TOTP recovery, and a controlled first-Observer bootstrap. Demo seed accounts do not satisfy this gate. See `docs/product/corvinum-account-onboarding.md`. |
