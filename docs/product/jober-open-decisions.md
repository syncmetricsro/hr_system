...
| Project reassignment | Whether manager approval is required. | **Answered 2026-06-28** (phase1-open-questions Q2): **coordinator-driven** — no separate manager approval for assigning/reassigning a person to a project. (Initial activation to WORKING still runs through four-pillar readiness + manager approval, §12.4.) **Implemented 2026-07-27** as `ActivationApproval`: coordinator requests, manager decides, requester may not decide their own request. |
...
| Offer emails: inactive pool | Whether a job offer may be emailed to people in **INACTIVE** state (the recycling pool), or only to AVAILABLE. | **Open (raised 2026-08-03, ADR 0029).** The build permits it — bulk send filters by lifecycle status but does not exclude INACTIVE — and nothing in the lifecycle model decides it. Opt-out and blacklist are refused regardless. |
| Offer emails: sender address | Jober `noreply@` address for outbound offer emails. | **Not supplied (2026-08-03).** Jober email configuration deferred until a demo is scheduled; the feature reports itself unavailable until the `DJANGO_EMAIL_*` vars and `EMAIL_ALLOWED_RECIPIENTS` are set. No code change pending. |
