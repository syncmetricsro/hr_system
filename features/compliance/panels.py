...
from features.compliance.models import CertificateCategory
from features.compliance.services import _severity, compliance_alerts, most_relevant_certificate
...
def certificate_badges(request, person):
    ...
    for category, certs in by_category.items():
        ...
        if best.expiry_date:
            tooltip = _("%(name)s (expires %(date)s)") % {"name": best.name, "date": best.expiry_date}
        else:
            tooltip = best.name
        badges.append({
...
def certificate_panel(request, person):
    ...
    for certificate in certificates:
        certificate.severity = _severity(certificate.expiry_date, today, 30) if certificate.expiry_date else None
