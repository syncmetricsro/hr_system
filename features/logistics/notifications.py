...
def logistics_notification_provider(request):
    if not flag_enabled("equipment") or not can(
        request.user, Action.EQUIPMENT_REVIEW_DEDUCTION
    ):
        return []
    issues = list(pending_deduction_reviews(request.user)["issues"])
    if not issues:
        return []
    state = "|".join(
        f"{issue.pk}:{issue.review_status}:{issue.charge_amount}" for issue in issues
    )
    return [
        NotificationItem(
            ...
            detail=str(
                _("%(count)s unreturned equipment reviews") % {"count": len(issues)}
            ),
