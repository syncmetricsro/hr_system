...
def _non_negative(value) -> Decimal:
    ...
    if amount < 0:
        raise ValueError(
            _("Amounts use a positive convention; negative values are not allowed.")
        )
    return amount
...
def set_room_rate(room, rate, *, actor=None):
    ...
    record_event(
        actor, "accommodation.rate_set", target=room, rate=str(room.monthly_rate)
    )
    return room
...
def set_assignment_rate(assignment, rate, *, actor=None):
    ...
    record_event(
        actor,
        "accommodation.assignment_rate_set",
        target=assignment,
        rate=(
            "" if assignment.rate_override is None else str(assignment.rate_override)
        ),
    )
    return assignment
...
def accommodation_cost_report():
    ...
    for accommodation in Accommodation.objects.prefetch_related("rooms__assignments"):
        ...
        assigned_cost = sum(
            (assignment.effective_rate for assignment in active), Decimal("0")
        )
        row = {
            "accommodation": accommodation,
            "rooms": len(rooms),
...
@transaction.atomic
def set_accommodation_cost_period(
    accommodation, *, effective_month, capacity, per_head_cost, actor=None
):
    period, _created = AccommodationCostPeriod.objects.update_or_create(
        ...
        defaults={
            ...
            "per_head_cost": _non_negative(per_head_cost),
...
def set_assignment_payment(assignment, amount, *, actor=None):
    ...
    record_event(
        actor,
        "accommodation.worker_payment_set",
        target=assignment,
...
def assign_room(person, room, *, actor=None, worker_payment_monthly=0):
    ...
    active = room.assignments.filter(status=RoomAssignmentStatus.ACTIVE).exclude(
        person=person
    )
...
def release_room(person, *, actor=None):
    today = timezone.localdate()
    assignment = person.room_assignments.filter(
        status=RoomAssignmentStatus.ACTIVE
    ).first()
...
def flag_unreturned(issue, *, actor=None):
    ...
    record_event(
        actor,
        "equipment.flagged_unreturned",
        target=issue,
        amount=str(issue.charge_amount),
    )
    return issue
...
def review_deduction(issue, decision, *, actor=None, note: str = ""):
    ...
    issue.save(
        update_fields=["review_status", "reviewed_by", "reviewed_at", "review_note"]
    )
...
def _offices_in_use() -> bool:
    """Whether this install populates offices at all (ADR 0026 Phase B).

    Mirrors ``user_office_scope``'s data-driven check rather than branching on
    the client: CorvinumEU never creates ``Office`` rows, so its equipment
    stock stays a single pooled ledger with no office dimension.
    """
...
def _consume_fifo(
    *,
    ...
    """FIFO draw. When ``office`` is given, only that office's lots are
    eligible - there is no cross-office draw (ADR 0026 decision point 6), so
    an office short on stock raises rather than silently consuming another
    warehouse's batch. ``office=None`` keeps the original pooled behavior for
    installs that never populate offices at all.
    """
    lots = EquipmentStockLot.objects.select_for_update().filter(
        item=item, remaining_quantity__gt=0
    )
    if office is not None:
        lots = lots.filter(office=office)
    lots = list(lots.order_by("received_on", "id"))
    available = sum(lot.remaining_quantity for lot in lots)
    if available < quantity:
        if office is not None:
            raise LogisticsWorkflowError(
                _("Only %(available)s units are available in %(office)s's stock.")
                % {"available": available, "office": office.name}
            )
        raise LogisticsWorkflowError(
            _("Only %(available)s units are available in stock.")
            % {"available": available}
        )
    remaining = quantity
    allocations = []
    ...
    for lot in lots:
        ...
        if take == lot.remaining_quantity:
            ...
            value = (
                lot.remaining_value * Decimal(take) / Decimal(lot.remaining_quantity)
...
def receive_stock(
    *,
    ...
    for item, quantity, value in prepared:
        ...
        _create_inbound_lot(
            item=item,
            movement_type=EquipmentStockMovementType.RECEIPT,
...
@transaction.atomic
def adjust_stock(
    item, quantity_delta, *, occurred_on, value=None, reason, actor=None, office=None
):
    quantity_delta = int(quantity_delta)
    if not reason.strip() or quantity_delta == 0:
        raise LogisticsWorkflowError(
            _("A non-zero adjustment and reason are required.")
        )
    if quantity_delta > 0:
...
def equipment_stock_balance(*, as_of=None, offices=None):
    """``offices=None`` means unfiltered (ADR 0026 convention, matching
    finance's services) - pass a scope queryset to restrict to those offices."""
    qs = EquipmentStockMovement.objects.select_related("item")
    if as_of is not None:
        qs = qs.filter(occurred_on__lte=as_of)
    if offices is not None:
        qs = qs.filter(office__in=offices)
    grouped = (
        qs.values("item_id", "item__name", "item__size")
        .annotate(quantity=Sum("quantity_delta"), value=Sum("value_delta"))
...
def issue_equipment(person, item, quantity=1, *, actor=None, operation_key=None):
    if not item.is_active:
        raise LogisticsWorkflowError(
            _("This equipment item is inactive and cannot be issued.")
        )
    if stock_ledger_enabled() and operation_key is None:
        raise LogisticsWorkflowError(
            _("An operation key is required for stock-tracked issuance.")
        )
    # ADR 0026 Phase B: a stock-tracked issue draws from the person's own
    # office's warehouse. Reject rather than silently drawing from another
    # office when the person has no office set - only when offices are
    # actually in use (an install with none keeps the pooled behavior).
    office = None
    if stock_ledger_enabled() and _offices_in_use():
        office = person.office
        if office is None:
            raise LogisticsWorkflowError(
                _("Set this person's office before issuing stock-tracked equipment.")
            )
    if operation_key:
    ...
    if stock_ledger_enabled():
        _movement, value = _consume_fifo(
            item=item,
            quantity=quantity,
            movement_type=EquipmentStockMovementType.ISSUE,
...
@transaction.atomic
def record_transport_week(
    project, week_start, headcount, *, actor=None, note: str = ""
):
...
@transaction.atomic
def create_transport_week(
    *, project, week_start, headcount, note: str = "", actor=None
):
    if TransportWeek.objects.filter(project=project, week_start=week_start).exists():
        raise LogisticsWorkflowError(
            _("A transport record already exists for this project and week.")
        )
...
@transaction.atomic
def update_transport_week(
    week, *, project, week_start, headcount, note: str = "", actor=None
):
    duplicate = TransportWeek.objects.filter(
...
def save_accommodation(accommodation, *, actor=None, old=None):
    ...
    record_event(
        actor,
        "accommodation.updated" if old else "accommodation.created",
...
def save_room(room, *, actor=None, old=None):
    ...
    record_event(
        actor,
        "room.updated" if old else "room.created",
        target=room,
        old=old or {},
        new={
            "accommodation": room.accommodation_id,
            "label": room.label,
            "capacity": room.capacity,
            "monthly_rate": str(room.monthly_rate),
...
def return_equipment(issue, *, actor=None, disposition=None):
    ...
    if stock_ledger_enabled() and disposition not in {"restock", "retire"}:
        raise LogisticsWorkflowError(
            _("Choose whether the returned equipment is reusable or retired.")
        )
    ...
    if stock_ledger_enabled() and disposition == "restock":
        value = (
            issue.issued_stock_value or Decimal(issue.item.unit_price) * issue.quantity
        )
        # Restocks into the person's *current* office. No office snapshot is
        # kept on EquipmentIssue (matching the design doc's field list), so a
        # person who changed office between issue and return restocks into the
        # new one - accepted as low-probability rather than adding schema.
        _create_inbound_lot(
            item=issue.item,
            movement_type=EquipmentStockMovementType.RETURN,
            occurred_on=timezone.localdate(),
            quantity=issue.quantity,
            value=value,
            actor=actor,
            issue=issue,
            office=issue.person.office if _offices_in_use() else None,
        )
    record_event(
        actor, "equipment.returned", target=issue, disposition=issue.return_disposition
    )
    return issue
