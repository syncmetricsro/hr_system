...
class Accommodation(models.Model):
    ...
    # Office scoping (ADR 0026 Phase B). Rooms have no separate office field —
    # they FK to one Accommodation and inherit it transitively.
...
class AccommodationCostPeriod(models.Model):
    ...
    recorded_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="accommodation_cost_periods",
...
class RoomAssignment(models.Model):
    ...
    status = models.CharField(
        _("status"),
        max_length=20,
        choices=RoomAssignmentStatus.choices,
        default=RoomAssignmentStatus.ACTIVE,
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                ...
                name="unique_active_room_per_person",
            )
        ]
...
class EquipmentIssue(models.Model):
    ...
    status = models.CharField(
        _("status"),
        max_length=20,
        choices=EquipmentIssueStatus.choices,
        default=EquipmentIssueStatus.ISSUED,
    )
    ...
    review_status = models.CharField(
        _("review status"),
        max_length=20,
        choices=DeductionReviewStatus.choices,
        default=DeductionReviewStatus.NONE,
    )
    ...
    reviewed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="reviewed_equipment_deductions",
    ...
    operation_key = models.UUIDField(
        _("operation key"), null=True, blank=True, unique=True
    )
    issued_stock_value = models.DecimalField(
        _("issued stock value"),
        max_digits=12,
        decimal_places=2,
        null=True,
        blank=True,
    ...
    return_disposition = models.CharField(
        _("return disposition"), max_length=20, blank=True
    )
...
class EquipmentStockReceipt(models.Model):
    operation_key = models.UUIDField(
        _("operation key"), default=uuid4, unique=True, editable=False
    )
    ...
    recorded_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="equipment_stock_receipts",
        verbose_name=_("recorded by"),
    )
    created_at = models.DateTimeField(_("created"), auto_now_add=True)
...
class EquipmentStockReceiptLine(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=("receipt", "item"), name="unique_item_per_stock_receipt"
            ),
        ]
...
class EquipmentStockMovement(models.Model):
    ...
    movement_type = models.CharField(
        _("movement type"),
        max_length=20,
        choices=EquipmentStockMovementType.choices,
    )
...
class EquipmentStockLot(models.Model):
    ...
    remaining_value = models.DecimalField(
        _("remaining value"), max_digits=12, decimal_places=2
    )
...
class EquipmentStockAllocation(models.Model):
    ...
    value = models.DecimalField(
        _("value"), max_digits=12, decimal_places=2, validators=NON_NEGATIVE
    )

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=("outbound_movement", "lot"), name="unique_stock_allocation_lot"
            ),
        ]
...
class TransportWeek(models.Model):
    ...
    recorded_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="recorded_transport_weeks",
        verbose_name=_("recorded by"),
    )
    created_at = models.DateTimeField(_("created"), auto_now_add=True)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name="unique_transport_week_per_project",
            )
        ]
