...
# demo users + people + questionnaire + finance, serves the app, and runs the
# whole tests/e2e suite (shell smoke + feature pages) against it.
#
...
for i in $(seq 1 30); do
  if docker exec "$DB" pg_isready -U jober -d jober >/dev/null 2>&1; then break; fi
  sleep 1
...
with urllib.request.urlopen("http://$APP:8000/healthz/", timeout=2) as r:
    raise SystemExit(0 if r.status == 200 and r.read().decode() == "ok" else 1)
PY
  then break; fi
  sleep 1
  [[ "$i" = "30" ]] && { docker logs "$APP"; exit 1; }
done
