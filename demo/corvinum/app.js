(() => {
  ...
  const translations = {
    sk: {
      ...
      "Can manage transport logistics only.": "Môže spravovať iba dopravnú logistiku.",
    ...
    hu: {
      ...
      "Can manage transport logistics only.": "Csak szállítási logisztikát kezelhet.",
  ...
  const coordinatorViews = new Set(["shifts", "sms", "second-shift"]);
  ...
  function visibleNavItems() {
    return navItems.filter((item) => isViewAllowed(item.view));
  }
  ...
  function roleDescription() {
    ...
    if (isCoordinator()) {
      return t("Can manage transport logistics only.");
    }
  ...
  function renderManifestRail() {
    ...
    return `
      <aside class="manifest-rail" aria-label="Guided demo manifest">
        ...
        <p class="rail-subtitle">${escapeHtml(t(isCoordinator() ? "Logistics-only journey." : "Eleven-stop worker journey."))}</p>
