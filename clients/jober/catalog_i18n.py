"""Translatable msgids for Jober's seeded catalog data.

The demo seed (clients/jober/demo/…) stores these canonical English strings in
the DB; templates render them through ``db_trans``. They are registered HERE
because makemessages ignores ``demo`` paths (scripts/compile_messages.sh) —
keep this list in sync with the seed.

**The nine strings below are byte-identical to CorvinumEU's on purpose**
(clients/corvinum_eu/catalog_i18n.py). ``db_trans`` is a verbatim lookup, so
identical text shares one catalog entry and one translation in each language.
Either client may reword its own list — that is why the list is duplicated
rather than shared — but the moment a string here differs by one character it
becomes a new msgid and needs sk, hu and uk before it ships.
"""
